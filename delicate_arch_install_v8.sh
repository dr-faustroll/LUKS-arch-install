...
# ---------- 9️⃣ LUKS header backup ----------
info "Backing up LUKS header to the new system's Documents folder..."
# Ensure the mount point exists first so we can write the backup there later
mkdir -p /mnt/root/Documents
# The actual backup will be performed *after* the root filesystem is created and mounted
# (see step 13 after mounting). We just note the intention here.
...
# ---------- 16️⃣ Capture LUKS UUID (used in boot entry) ----------
LUKS_UUID=$(blkid -s UUID -o value "$LUKS_PART")
[[ -n "$LUKS_UUID" ]] || die "Unable to obtain LUKS UUID."

# ---------- 17️⃣ Backup LUKS header (now that /mnt is mounted) ----------
