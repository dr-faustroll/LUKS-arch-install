...
# Dual‑boot desktop (Intel i9‑13900K + NVIDIA RTX‑4090)
# ------------------------------------------------------------
set -euo pipefail
IFS=$'\n\t'
...
# ---------- 2️⃣ Bootstrap – fetch the real installer ----------
OWNER="dr-faustroll"
REPO="LUKS-arch-install"
PATH_IN_REPO="delicate_arch_install_v11.sh"   # adjust if renamed
BRANCH="main"
TMP_SCRIPT="/tmp/${PATH_IN_REPO}"

# ---------- Public repository - no token needed ----------

if [[ -z "${BOOTSTRAPPED:-}" ]]; then
  info "Attempting to fetch the latest installer from GitHub..."
  DOWNLOAD_OK=false
  RAW_URL="https://raw.githubusercontent.com/${OWNER}/${REPO}/${BRANCH}/${PATH_IN_REPO}"

  if command -v curl >/dev/null 2>&1; then
    info "Using curl to download ${OWNER}/${REPO}:${BRANCH}/${PATH_IN_REPO}"
    curl -fsSL "$RAW_URL" -o "$TMP_SCRIPT" && DOWNLOAD_OK=true
  elif command -v wget >/dev/null 2>&1; then
    info "Using wget to download ${OWNER}/${REPO}:${BRANCH}/${PATH_IN_REPO}"
    wget -qO "$TMP_SCRIPT" "$RAW_URL" && DOWNLOAD_OK=true
  else
    warn "Neither curl nor wget is available – skipping remote fetch."
  fi

  if [[ "$DOWNLOAD_OK" == true ]]; then
    info "Fetched latest installer – handing off execution."
    chmod +x "$TMP_SCRIPT"
    export BOOTSTRAPPED=1
    exec /usr/bin/env bash "$TMP_SCRIPT"
  else
    warn "Download from public repo failed. Proceeding with embedded installer."
  fi
fi

# ---------- UEFI mode check ----------
# systemd-boot requires UEFI; fail early if not booted in UEFI mode.
[[ -d /sys/firmware/efi/efivars ]] || die "System is not booted in UEFI mode. Reboot the installer in UEFI."

# ---------- 3️⃣ Disk selection ----------
info "Available disks:"
mapfile -t DISKS < <(lsblk -dpno NAME,SIZE,MODEL,TYPE | awk '$4=="disk"{print NR")",$1,$2,$3}')
if (( ${#DISKS[@]} == 0 )); then
...
# ---------- 1️⃣0️⃣ LUKS header backup (deferred) ----------
...
# ---------- 1️⃣3️⃣ Encrypted swapfile (4 GiB) ----------
...
# ---------- 1️⃣7️⃣ Capture LUKS UUID (used in boot entry) ----------
LUKS_UUID=$(blkid -s UUID -o value "$LUKS_PART")
[[ -n "$LUKS_UUID" ]] || die "Unable to obtain LUKS UUID."

# ---------- 1️⃣8️⃣ Backup LUKS header (now that /mnt is mounted) ----------
